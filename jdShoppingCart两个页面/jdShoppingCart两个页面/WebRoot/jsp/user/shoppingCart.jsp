...
<script type="text/javascript">
			...
			$(document).ready(function() {
				...
				$('#submit-btn').on('click', function() {//获得登录按钮
					...
					layer.open({
						...
						shade: 0.8,
						 shift: 'right-bottom', 
						area: ['380px', '75%'],
						content: 'smalllogin.jsp'
							//content: '../JSHtml/DZSC_Protocol.html'
					});
  ...
  <body style="height: 100%">
  	...
  	<!--最顶上的一条开始-->
		...
		<!--最顶上的一条结束-->
    		...
    		<!-- 京东图标 -->
    		<div class="logo">
    			<div style="width: 270px;float: left;position: relative;"><a href="//www.jd.com/" class="link1"><img src="img/images2/jd.png" alt="京东商城"/></a>
    		...
    		<div class="div3">
    			<div class="cart-checkbox">
					<input checked="checked" id="selectAll2"class="jdcheckbox"type="checkbox" style="position: relative;top:1px;">
    		...
    		<div class="goods">
    			<!--京东自营那一条-->
    			<div class="shop">
	    			<input checked="checked" name="checkShop" id="selectShop" class="jdcheckbox self-shop " type="checkbox">
    		...
    		<%
				...
				double allProductMoney = 0;
				while(it.hasNext()){
					Product product = it.next();
					double singleProductMoney = 0;
					int productSum = product.getProductSum();
					double price = product.getPrice();
					singleProductMoney = price*productSum;
					allProductMoney+=singleProductMoney;
			%>
				<!--单品-->
    		<div class="buygoods buygoodsselected item-list">
    				<input type="hidden" id="productId" value="<%=product.getPid()%>"/>
					<!--图片及名称-->
	    			<div class="p-checkbox cell">
						...
						<!-- 手机图片 -->
						<div class="p-img">
							<a target="_blank" ><img style="width:60px;"alt="小米 Note 全网通 黑色 移动联通电信4G手机 双卡双待"src="img/userLoadUpProduct/<%=product.getShoppingCarImg() %>" /></a>
						</div>
						<div class="p-name">
						...
						<div class="p-extend">
							...
							<div class="cell p-price p-div3" ><strong><%=product.getPrice() %></strong></div>
							<div class="quantity-form">
									<button class="decrement">-</button>
									<input value="<%=product.getProductSum() %>" type="text" autocomplete="off" class="p-quantity-itxt" id="decreamentOne"/>
									<button class="increment">+</button>
							</div>
				
							<div class="cell p-sum div4"><strong><%=singleProductMoney %></strong></div>	
			...
			$(".decrement").click(function(){
				var num = $(this).next("#decreamentOne").val();
				var pid = $(this).parents(".buygoods").find("#productId").val();
				 if(num >1){
					//发送ajax请求   发给 ProductServlet
					$.post("ShoppingCartServlet",{
						"action":"decreamentOne",
						"pid":pid
					},function(data,status){
						var input = "<input value="+data[0].productSum+" type='text' autocomplete='off' class='p-quantity-itxt' id='decreamentOne'/>";
						$(this).next("#decreamentOne").replaceWith(input);
					},"json");
				} 
				
			});
			$(".increment").click(function(){
				var num = $(this).prev("#decreamentOne").val();
				num = parseInt(num);
				$(this).prev("#decreamentOne").val(1+num);
			});
